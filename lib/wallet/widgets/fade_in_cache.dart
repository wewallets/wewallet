...
class FadeInImage extends StatelessWidget {
  ...
  FadeInImage.assetNetwork(
      ...
      : assert(placeholder != null),
        assert(image != null),
        placeholder = placeholderScale != null ? ExactAssetImage(placeholder, bundle: bundle, scale: placeholderScale) : AssetImage(placeholder, bundle: bundle),
        assert(imageScale != null),
        assert(fadeOutDuration != null),
        assert(fadeOutCurve != null),
        assert(fadeInDuration != null),
...
class MyNetworkImage extends ImageProvider<MyNetworkImage> {
  ...
  MyNetworkImage(this.url, {this.scale = 1.0, this.headers, this.sdCache})
      : assert(url != null),
        assert(scale != null);
  ...
  static StreamController<ImageChunkEvent> chunkEvents = StreamController<ImageChunkEvent>();

  static void saveToImages(String url, String name) async {
  ...
  Future<Codec> _loadAsync(
    ...
    try {
      ...
      if (sdCache == true) {
        final Uint8List bytes = await getFromSdcard(key.url);
      ...
      if (sdCache == true && bytes.lengthInBytes != 0) {
        saveToImage(bytes, key.url);
      }
  ...
  @override
  ImageStreamCompleter load(MyNetworkImage key, decode) {
...
saveToImage(Uint8List mUint8List, String name) async {
  name = md5.convert(convert.utf8.encode(name)).toString();
...
getFromSdcard(String name) async {
  name = md5.convert(convert.utf8.encode(name)).toString();
  Directory dir = await getApplicationDocumentsDirectory();
  String path = dir.path + "/" + name;
  var file = File(path);
