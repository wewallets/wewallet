...
class DepthChart extends StatefulWidget {
  ...
  DepthChart(this.bids, this.asks, {this.fixedLength = 2, this.buyPathColor, this.sellPathColor});
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: CustomPaint(
        ...
        painter: DepthChartPainter(widget.bids, widget.asks, pressOffset, isLongPress, widget.fixedLength, widget.buyPathColor, widget.sellPathColor),
      ),
    );
  }
}
...
class DepthChartPainter extends CustomPainter {
  ...
  double mWidth = 0.0, mDrawHeight = 0.0, mDrawWidth = 0.0, mBuyPointWidth, mSellPointWidth;
  ...
  DepthChartPainter(this.mBuyData, this.mSellData, this.pressOffset, this.isLongPress, this.fixedLength, this.mBuyPathColor, this.mSellPathColor) {
    ...
    mBuyPathPaint ??= Paint()
      ..isAntiAlias = true
      ..color = mBuyPathColor == null ? ChartColors.depthBuyColor.withOpacity(0.2) : mBuyPathColor;
    mSellPathPaint ??= Paint()
      ..isAntiAlias = true
      ..color = mSellPathColor == null ? ChartColors.depthSellColor.withOpacity(0.2) : mSellPathColor;
    mBuyPath ??= Path();
    mSellPath ??= Path();
    init();
  }
...
//     canvas.drawColor(Colors.green, BlendMode.srcATop);
  ...
  void drawText(Canvas canvas) {
    ...
    for (int j = 0; j < mLineCount; j++) {
      ...
      str = NumUtil.formatNum(value.toString(), 4).toString();
      var tp = getTextPainter(str);
      tp.layout();
      tp.paint(canvas, Offset(mWidth - tp.width, mDrawHeight / mLineCount * j + tp.height / 2));
    }

    var startText = NumUtil.formatNum(mBuyData.first.price.toString(), 4).toString();
    ...
    var center = NumUtil.formatNum(centerPrice.toString(), 4).toString();
    ...
    var endText = NumUtil.formatNum(mSellData.last.price.toString(), 4).toString();
    TextPainter endTP = getTextPainter(endText);
    endTP.layout();
    endTP.paint(canvas, Offset(mWidth - endTP.width, getBottomTextY(endTP.height)));

    var leftHalfText = NumUtil.formatNum(((mBuyData.first.price + centerPrice) / 2).toString(), 4).toString();
    ...
    var rightHalfText = NumUtil.formatNum(((mSellData.last.price + centerPrice) / 2).toString(), 4).toString();
  ...
  void drawSelectView(Canvas canvas, int index, bool isLeft) {
    ...
    //画底部
    TextPainter priceTP = getTextPainter(NumUtil.formatNum(entity.price.toString(), 4), Colors.white);
    priceTP.layout();
    double left;
    ...
    //画左边
    TextPainter amountTP = getTextPainter(NumUtil.formatNum(entity.vol.toString(), 4), Colors.white);
    amountTP.layout();
  ...
  getTextPainter(String text, [Color color = ChartColors.defaultTextColor]) => TextPainter(text: TextSpan(text: "$text", style: TextStyle(color: color, fontSize: ScreenUtil().setSp(20))), textDirection: TextDirection.ltr);
