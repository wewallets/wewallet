...
class DateUtil {
  ...
  /// dateSeparate    date separate.
  /// timeSeparate    time separate.
  static String getDateStrByMs(int milliseconds, {DateFormat format = DateFormat.NORMAL, String dateSeparate, String timeSeparate, bool isUtc = false}) {
  ...
  //时间戳转日期
  static String formatDateFromMillisecondsSinceEpoch(String time, {String format = 'yyyy-MM-dd HH:mm'}) {
    return formatDateStr(DateTime.fromMillisecondsSinceEpoch(int.parse(time)).toString(), format: format);
  }
  ...
  // 格式化时间
  static String formatT(String t,{String format = 'HH:mm:ss'}){
    if(t==null){
      return '';
    }
    return DateUtil.formatDateMs(num.parse(t),format: format);
  }
}
