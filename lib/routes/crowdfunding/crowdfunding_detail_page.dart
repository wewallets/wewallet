...
//众筹详情
class CrowdFundingDetailPage extends StatefulWidget {
  final Bundle bundle;

  CrowdFundingDetailPage(this.bundle);

  @override
  _CrowdFundingDetailState createState() => _CrowdFundingDetailState();
}

class _CrowdFundingDetailState extends State<CrowdFundingDetailPage> {
  int type = 0; //0即将开始 1火爆进行中 2圆满收官 3失败项目
  WheelDetailEntity wheelDetailEntity;
    ...
    return Scaffold(
      ...
      body: wheelDetailEntity == null
          ...
          : ListView(children: [
              ...
              Container(width: double.infinity, height: adaptationDp(10), color: Color(0xFFF3F3F3)),
              Padding(padding: EdgeInsets.all(adaptationDp(15)), child: Text('${getString().xtj1}', style: TextStyles.textBlack15)),
              Gaps.vGap5,
              Padding(
                  padding: EdgeInsets.only(left: adaptationDp(15), bottom: adaptationDp(10)),
                  child: Row(
                    children: [
                      Text('${wheelDetailProductList.platformBonusAmount} ${wheelDetailProductList.currency.toUpperCase()}', style: TextStyles.textGrey614.copyWith(color: Color(0xFF3250D4))),
                    ],
                  )),
              Gaps.vGap10,
              Container(width: double.infinity, height: adaptationDp(10), color: Color(0xFFF3F3F3)),
              Padding(padding: EdgeInsets.all(adaptationDp(15)), child: Text('${getString().xtj3}', style: TextStyles.textBlack15)),
              listViewBuilder(
                  isSlide: true,
                  itemBuilder: (context, index) {
                    return buildList(index);
                  },
                  itemCount: wheelDetailEntity.productList.length),
              Gaps.vGap30,
            ]),
    );
  }
  ...
  buildList(index) {
    var indexData = wheelDetailEntity.productList[index];
    if (indexData.raiseStatus == '0')
        ...
        inkButton(
          child: Container(
              ...
              child: Column(
                children: [
                  Row(
                    children: [Gaps.hGap15, Text('${getString().zf55}${indexData.number}${getString().zf56}', style: TextStyles.textBlack12)],
                  ),
                  Gaps.vGap5,
                  Container(
                  ...
                  Row(
                    children: [
                      ...
                      Expanded(
                          child: inkButton(
                              onPressed: () {
                                navigatorTransactionContextPush(context, PageTransactionRouter.crowdfunding_product_detail_page, bundle: Bundle()..putObject('wheelDetailProductList', indexData));
                              },
                              child: Container(
                                decoration: BoxDecoration(color: Colours.white, borderRadius: BorderRadius.circular(adaptationDp(5)), border: Border.all(color:Colours.colorFFC939F3, width: 0.5)),
                                padding: EdgeInsets.only(left: adaptationDp(50), top: adaptationDp(5), bottom: adaptationDp(5), right: adaptationDp(50)),
                                alignment: Alignment.center,
                                child: Text('${getString().zf13}', style: TextStyles.textBlack12.copyWith(color:Colours.colorFFC939F3)),
                              ))),
                      Gaps.hGap15,
                    ],
                  ),
      ...
      return inkButton(
          ...
          child: Container(
            ...
            child: Column(children: [
              Row(
                children: [
                  Text('${getString().zf55}${indexData.number}${getString().zf56}', style: TextStyles.textBlack12),
                  Gaps.hGap5,
  ...
  getData() {
    Net().post(ApiTransaction.fund_detail, {'wheel_id': widget.bundle.getString('productId')}, success: (data) {
      wheelDetailEntity = WheelDetailEntity().fromJson(data);
      wheelDetailProductList = wheelDetailEntity.productList[0];
