...
class TimeFormat {
  static const List<String> YEAR_MONTH_DAY = [yyyy, '-', mm, '-', dd, ' ', HH, ':', nn];
...
class _KChartWidgetState extends State<KChartWidget> with TickerProviderStateMixin {
  ...
  bool isScale = false, isDrag = false, isLongPress = false, isLoadMore = false;
  ...
  Widget build(BuildContext context) {
    if (widget.datas == null || widget.datas.isEmpty) {
      ...
      mScaleX = 1.0;
      isLoadMore = false;
    } else if (widget.datas.length > 40) {
      isLoadMore = true;
    }
  ...
  void _onFling(double x) {
    ...
    aniX.addListener(() {
      mScrollX = aniX.value;
      if (mScrollX <= 50 && widget.datas != null && widget.datas.length > 40) {
        isLoadMore = true;
      } else {
        isLoadMore = false;
      }
    ...
    return StreamBuilder<InfoWindowEntity>(
        ...
        builder: (context, snapshot) {
          ...
          infos = [
            ...
            entity.rice.toString(),
            entity.rice_fall.toString()+'%',
            NumUtil.formatNum(entity.amount.toString(), 4).toString(),
          ];
          return Container(
            margin: EdgeInsets.only(left: snapshot.data.isLeft ? 4 : mWidth - mWidth / 3 - 4, top: 25),
            width: mWidth / 3,
            decoration: BoxDecoration(color: Color(0xFF141F31), border: Border.all(color: Color(0xFF3E4858), width: 0.5), borderRadius: BorderRadius.all(Radius.circular(6))),
  ...
  Widget _buildItem(String info, String infoName) {
    ...
    return Row(
      ...
      children: <Widget>[
        Expanded(child: Text("$infoName", style: const TextStyle(color: ChartColors.defaultTextColor, fontSize: 10.0))),
